...
```
  ...
  When working under a Bayesian framework, the posterior distribution is the objective function of the analysis since it comprises all the information about the parameters of interests, those that describe the fundamental niche ($\mu$ and $A$).
  ...
  There is an R package that contains the t-walk algorithm, it is called "Rtwalk". I will use the function "Runtawlk" from this package to simulate from the posterior. In order to to that I have to define three main functions: Supp which checks that a new value in the Markov chain is valid, Initth which produces values from the _a priori_ distributions, and Energy which containd the posterior. I already defined and tested these functions as part of the assignement.  
  After simulating from the posterior, I need another set of functions to analyze and display the results. As an example, I defined the function "PlotIterations.R" which takes the output of the function "Runtwalk" and plots the resulting ellipses in the environmental space.