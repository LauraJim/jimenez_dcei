...
temp.cl <- cleanTP(dats=temp,minY=40)
temp.obs <- temp.cl[[1]]
temp.obs[1:8,1:8]
dim(temp.obs)
...
After applying the Mann-Kendall test for all the time series, i.e. for each weather station, we looked at the p-values and classified the values of the scores into three categories: significant negative trend (identified with a value of -1), non-significant trend (0), and significant positive trend (1). The following table shows the number of stations in each class for the temperature data:
`r table(tscores)`
where we can see that more than half of the stations showed a positive trend. The corresponding table for precipitation is
`r table(pscores)`
where also positive trends are common.

<!--- Chunk for plotting the scores from the Mann-Kendall test for all stations. Dependencies: tscores,pscores,temp.cl,prec.cl --->
```{r map-trends, dev='png', fig.cap="Figure 2. Some time series of temperature (top) and precipitation (bottom) from 1950 to 2016 that we got after cleaning the data."}
...
# plot point at the locations of the stations and color the points according to the score values
